...
class FPQuantizeFunction(InplaceFunction):
    ...
    def forward(ctx, input, num_bits=32, qparams=None, flatten_dims=_DEFAULT_FLATTEN,
        ...
        with torch.no_grad():
            output_stochastic = output.clone()
            rand_tmp = torch.cuda.FloatTensor(output.shape).uniform_(0, 1)
            output_stochastic.sub_(min_values).div_(delta).clamp_(qmin, qmax)
            output.sub_(min_values).div_(delta).clamp_(qmin, qmax).floor_()
            diff = output_stochastic - output
            rand_tmp = (rand_tmp < diff).float()
            output.add_(rand_tmp)
